...
const getterReturnRule = index_js_2.eslintRules['getter-return'];
...
exports.rule = {
    ...
    create(context) {
        ...
        return (0, index_js_1.mergeRules)(getterReturnListener, noAccessorFieldMismatchListener);
    },
};
...
function decorateGetterReturn(rule) {
    return (0, index_js_1.interceptReport)(rule, (context, descriptor) => {
...
function reportWithSonarFormat(context, descriptor, message) {
    (0, index_js_1.report)(context, { ...descriptor, message });
}
function reportWithSecondaryLocation(context, accessor) {
    ...
    (0, index_js_1.report)(context, {
        ...
        (0, index_js_1.toSecondaryLocation)(fieldToRefer.node, `${ref[0].toUpperCase()}${ref.slice(1)} which should be referred.`),
    ]);
}
function isPropertyDefinitionCall(call) {
    ...
    const minArgs = 3;
    return call && objects.some(object => (0, index_js_1.isMethodInvocation)(call, object, method, minArgs));
}
function isPropertiesDefinitionCall(call) {
    const object = 'Object';
    const methods = ['defineProperties', 'create'];
    const minArgs = 2;
    return call && methods.some(methodName => (0, index_js_1.isMethodInvocation)(call, object, methodName, minArgs));
}
...
function getSingleDescriptorAccessorInfo(accessorNode) {
    ...
    if (!isPropertyDefinitionCall(callNode) || !propertyNode || !(0, index_js_1.isStringLiteral)(propertyNode)) {
        return null;
    }
...
function getSingleVariableFieldMap(context, name, node) {
    ...
    for (const candidate of [name, `_${name}`, `${name}_`]) {
        const variable = (0, index_js_1.getVariableFromName)(context, candidate, node);
