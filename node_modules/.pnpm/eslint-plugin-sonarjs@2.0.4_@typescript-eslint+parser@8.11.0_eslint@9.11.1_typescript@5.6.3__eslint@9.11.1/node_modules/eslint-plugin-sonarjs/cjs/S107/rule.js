...
const eslintMaxParams = index_js_1.eslintRules['max-params'];
const DEFAULT_MAXIMUM_FUNCTION_PARAMETERS = 7;
const messages = { ...eslintMaxParams.meta?.messages };
exports.rule = {
    meta: (0, index_js_2.generateMeta)(meta_js_1.meta, { messages, schema: meta_js_1.schema }),
    create(context) {
        const decorationListeners = ruleDecoration.create(context);
        const extensionListeners = ruleExtension.create(context);
        return (0, index_js_2.mergeRules)(decorationListeners, extensionListeners);
    },
};
/**
 ...
 */
const ruleDecoration = (0, index_js_2.interceptReport)(eslintMaxParams, function (context, descriptor) {
    ...
    function isAngularConstructor(functionLike) {
        ...
        function isConstructor(node) {
            return (node?.type === 'MethodDefinition' && (0, index_js_2.isIdentifier)(node.key, 'constructor'));
        }
        function isAngularComponent(node) {
            return (node?.type === 'ClassDeclaration' &&
                node.decorators?.some(decorator => {
                    const node = decorator.expression;
                    return ((0, index_js_2.isFunctionCall)(node) &&
                        (0, index_js_2.getFullyQualifiedName)(context, node.callee) === '@angular.core.Component');
                }));
        }
    }
});
